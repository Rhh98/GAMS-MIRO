...
Set
   ...
   courn(I)  'cournot or price taker';
...
report(I,'nash') = q.l(I)*pr(q.l) - TC(I,q.l);
report('price','nash') = pr(q.l);

model Stackelberg / defobj_Cour, defp / ;

loop(k,
  leader(k) = yes ;
  courn(i) = yes$sameas(i,k);
  q.l(i)=1;
  ...
  report(I,K) = q.l(I)*pr(q.l) - TC(I,q.l);
  report('price',K) = pr(q.l);
  leader(k) = no ;
);
